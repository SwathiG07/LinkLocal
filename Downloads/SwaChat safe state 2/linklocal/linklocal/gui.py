...
class LinkLocalGUI:
    ...
    def _build_chat_panel(self) -> None:
        ...
        tk.Button(toolbar, text="❌ Leave Group", command=self._delete_group_dialog, bg=c["composer"], fg="#ef4444",
                  relief=tk.FLAT, font=("Segoe UI", 9), padx=6, cursor="hand2").pack(side=tk.RIGHT, padx=2)
    ...
    def _open_settings_dialog(self) -> None:
        status = simpledialog.askstring("Settings", "Status message:", initialvalue=self.peer.config.get("status_message", "Available"), parent=self.root)
        if status is None:
            return
        avatar = simpledialog.askstring("Settings", "Avatar / initials:", initialvalue=self.peer.config.get("avatar", "LL"), parent=self.root) or self.peer.config.get("avatar", "LL")
        dnd = messagebox.askyesno("Settings", "Enable Do Not Disturb?", parent=self.root)
        theme = "dark" if messagebox.askyesno("Settings", "Use dark theme?", parent=self.root) else "light"
        self.peer.update_profile(status_message=status, avatar=avatar, do_not_disturb=dnd, theme=theme)
        if theme != self.theme_name:
            messagebox.showinfo("Theme Updated", "Restart the app to fully apply the new theme.", parent=self.root)
