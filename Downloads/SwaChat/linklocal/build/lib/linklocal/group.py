...
class GroupManager:
    ...
    async def _handle_group_meta(self, payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if action in {"rename_request", "promote_request", "kick_request", "announcement_request", "meta_request", "pin_request"} and group is not None:
            try:
                if action == "rename_request":
                    ...
                    payload_copy = self.peer._base_payload("group_meta", group_id=group_id)
                    payload_copy["action"] = "state_update"
                    payload_copy["group"] = group.to_dict()
                    payload_copy["group"]["members"] = [m for m in payload_copy["group"]["members"] if m["peer_id"] != payload["target_peer_id"]]
                    self._launch_async(self.peer.send(payload_copy, payload["target_peer_id"]), ignore_errors=True)
        ...
        if action == "state_update":
            ...
            server_group = Group.from_dict({}, payload["group"])
            if not server_group.is_member(self.peer.peer_id):
                self.groups.pop(group_id, None)
                storage.delete_group_meta(group_id)
                self.peer.event_queue.put({"event": "group_kicked", "data": {"group_id": group_id}})
                return {"ok": True}
            self.groups[group_id] = server_group
            self.peer.event_queue.put({"event": "group_changed", "data": {"group_id": group_id}})
            return {"ok": True}
    ...
    async def _handle_superuser_query(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if action == "kick_member":
            try:
                # Notify the kicked user securely before their internal pointer is dropped
                payload_copy = self.peer._base_payload("group_meta", group_id=group_id)
                payload_copy["action"] = "state_update"
                payload_copy.update({"group": self.get_group(group_id).to_dict()})
                payload_copy["group"]["members"] = [m for m in payload_copy["group"]["members"] if m["peer_id"] != payload["target_peer_id"]]
                self._launch_async(self.peer.send(payload_copy, payload["target_peer_id"]), ignore_errors=True)
