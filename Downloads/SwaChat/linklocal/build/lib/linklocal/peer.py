...
class Peer:
    ...
    async def _send_payload(
        ...
        *,
        ...
        for attempt in range(3):
            try:
                reader, writer = await asyncio.open_connection(peer_info["ip"], int(peer_info["tcp_port"]), limit=100*1024*1024)
                try:
                    ...
                    self._emit("stats", {"peer_id": to_peer_id, "stats": stats})
                    self._log("tcp_send", {"peer_id": to_peer_id, "type": payload.get("type"), "bytes": len(raw)})
        ...
        raise ConnectionError(str(last_error) if last_error else f"Unable to reach peer {to_peer_id}")
    ...
    def _handle_seen_payload(self, payload: Dict[str, Any]) -> None:
        ...
        data = {"message_id": payload["target_message_id"], "sender_id": payload["sender_id"], "group_id": payload.get("group_id")}
        self._emit("seen", data)
        return None
