...
class LinkLocalGUI:
    def __init__(self, name: str) -> None:
        ...
        self.message_widgets: Dict[str, Dict] = {}

        self._build_layout()
        self._bind_events()
        self._show_welcome_screen() # Start with welcome screen
    ...
    def _build_sidebar(self) -> None:
        ...
        for text, cmd in [("Dashboard", self._open_dashboard)]:
    ...
    def _build_chat_panel(self) -> None:
        ...
        self.leave_btn = tk.Button(toolbar, text="❌ Leave Group", command=self._delete_group_dialog, bg=c["composer"], fg="#ef4444",
                                  relief=tk.FLAT, font=("Segoe UI", 9), padx=6, cursor="hand2")
        
        self.block_btn = tk.Button(toolbar, text="🚫 Block", command=self._toggle_block_action, bg=c["composer"], fg="#ef4444",
                                  relief=tk.FLAT, font=("Segoe UI", 9), padx=6, cursor="hand2")

        # They start hidden and are packed by _open_conversation
    ...
    def _refresh_sidebar(self) -> None:
        ...
        discovered = self.peer.discovery.get_peers()
        ...
        for group in sorted(self.groups.list_groups(), key=lambda item: item.group_name.lower()):
            ...
            # Bind click on entire row
            for widget in [row, name_frame] + name_frame.winfo_children():
                widget.bind("<Button-1>", lambda e, gid=group.group_id, n=group.group_name: self._open_conversation("group", gid, n))
    ...
    def _open_conversation(self, kind: str, conversation_id: str, title: str) -> None:
        ...
        self.message_widgets = {} # Clear widget map for new chat
        self._mark_chat_as_seen()
        
        if hasattr(self, 'leave_btn'):
            if kind == "group":
                self.leave_btn.pack(side=tk.RIGHT, padx=2)
                self.leave_btn.configure(text="❌ Leave Group")
            else:
                self.leave_btn.pack_forget()
        
        if hasattr(self, 'block_btn'):
            if kind == "peer":
                self.block_btn.pack(side=tk.RIGHT, padx=2)
                is_blocked = conversation_id in self.peer.config.get("blocked_peers", [])
                self.block_btn.configure(text="🛡️ Unblock" if is_blocked else "🚫 Block")
            else:
                self.block_btn.pack_forget()
    ...
    def _mark_chat_as_seen(self) -> None:
        chat_id = self._current_chat_id()
        if not chat_id:
            return
        messages = storage.load_history(chat_id)
        for message in messages:
            if self.peer.peer_id not in message.get("seen_by", []):
                if self.current_kind == "group":
                    self._launch_async(self.peer.send_seen(message["message_id"], message["sender_id"], group_id=self.current_id), ignore_errors=True)
                elif self.current_id:
                    self._launch_async(self.peer.send_seen(message["message_id"], self.current_id), ignore_errors=True)

    def _show_welcome_screen(self) -> None:
        """Fill the blank space with a welcome dashboard."""
        c = self.colors
        ...
        self.title_var.set("Welcome to LinkLocal")
        self.pin_var.set("")
        
        frame = tk.Frame(self.messages_container, bg=c["panel"])
        frame.place(relx=0.5, rely=0.4, anchor="center")
        
        tk.Label(frame, text="🛰️", font=("Segoe UI Emoji", 48), bg=c["panel"]).pack()
        tk.Label(frame, text="LinkLocal", font=("Segoe UI", 24, "bold"), bg=c["panel"], fg=c["accent"]).pack(pady=10)
        tk.Label(frame, text="Your decentralized communication hub is ready.", font=("Segoe UI", 11), bg=c["panel"], fg=c["muted"]).pack()
        
        stats_frame = tk.Frame(frame, bg=c["panel"], pady=20)
        stats_frame.pack()
        
        peers_count = len(self.peer.discovery.get_peers())
        tk.Label(stats_frame, text=f"Active Peers: {peers_count}", font=("Segoe UI", 10, "bold"), bg=c["panel"], fg=c["text"]).pack(side=tk.LEFT, padx=15)
        
        my_id_short = self.peer.peer_id[:8] + "..."
        tk.Label(stats_frame, text=f"Your ID: {my_id_short}", font=("Segoe UI", 10, "bold"), bg=c["panel"], fg=c["text"]).pack(side=tk.LEFT, padx=15)

        tk.Label(frame, text="Select a conversation from the left to start chatting.", 
                 font=("Segoe UI", 10, "italic"), bg=c["panel"], fg=c["muted"]).pack(pady=30)
        
        self._refresh_sidebar()

    def _render_messages(self, messages: Optional[list] = None) -> None:
        c = self.colors
        # If no messages provided, full reload (e.g. when opening a new chat)
        if messages is None:
            for widget in self.messages_container.winfo_children():
                widget.destroy()
        ...
        for message in messages:
            if message["message_id"] in self.message_widgets:
                self._update_message_status(message)
                continue
            self._draw_message(message, lookup)
            
        # Scroll to bottom
        self.root.update_idletasks()
        self.canvas.yview_moveto(1.0)

    def _draw_message(self, message: Dict, lookup: Dict) -> None:
        c = self.colors
        ...
        bubble = tk.Frame(outer, bg=bg, bd=0, padx=14, pady=10)
        ...
        meta_frame = tk.Frame(header, bg=bg)
        meta_frame.pack(side=tk.RIGHT)
        
        meta = format_timestamp(message["timestamp"])
        if message.get("is_edited"):
            meta += " · edited"
        tk.Label(meta_frame, text=meta, bg=bg, fg=c["muted"],
                 font=("Segoe UI Emoji", 8)).pack(side=tk.LEFT)
                 
        tick_label = None
        if is_self:
            tick_label = tk.Label(meta_frame, text=" ✓", bg=bg, fg=c["muted"], font=("Segoe UI Emoji", 9))
            tick_label.pack(side=tk.LEFT)
            self._apply_tick_styles(tick_label, message)
        ...
        body_lbl = tk.Label(bubble, text=body_text, bg=bg, fg=body_fg, font=body_font,
                        justify=tk.LEFT, wraplength=440)
        body_lbl.pack(anchor="w", pady=(4, 0))
        ...
        if file_info:
            ...
            if saved_path and Path(saved_path).exists():
                tk.Button(bubble, text="⬇️ Save As...", 
                          command=lambda p=saved_path, n=file_info.get("filename", "file"): self._save_file_as(p, n),
                          bg=c["accent"], fg="white", relief=tk.FLAT, font=("Segoe UI", 8, "bold"), 
                          padx=6).pack(anchor="w", pady=(4, 0))
        ...
        if message.get("poll"):
            ...
            tk.Label(poll_card, text=f"📊 {poll_data['question']}", bg=bg, fg=c["text"],
                     font=("Segoe UI", 10, "bold"), anchor="w", wraplength=380).pack(fill=tk.X, pady=(0, 4))
            for option in poll_data["options"]:
                ...
                lbl = tk.Label(opt_frame, text=f"{'●' if is_mine else '○'} {option} ({pct}%)", 
                               bg=bg, fg=c["text"], font=("Segoe UI", 9), anchor="w")
                lbl.pack(fill=tk.X)
                lbl.bind("<Button-1>", lambda e, opt=option, msg=message: self._vote_on_poll(msg, opt))
        ...
        if message.get("reactions"):
            ...
            for emoji, peer_ids in message["reactions"].items():
                tk.Label(reaction_bar, text=f"{emoji} {len(peer_ids)}", bg=bg, fg=c["text"],
                         font=("Segoe UI Emoji", 9)).pack(side=tk.LEFT, padx=2)
        
        # Track widgets
        self.message_widgets[message["message_id"]] = {
            "tick": tick_label,
            "body": body_lbl
        }
        bubble.bind("<Button-3>", lambda e, msg=message: self._open_context_menu(e, msg))
        body_lbl.bind("<Button-3>", lambda e, msg=message: self._open_context_menu(e, msg))

    def _apply_tick_styles(self, label: tk.Label, message: Dict) -> None:
        if not label: return
        seen_by = message.get("seen_by", [])
        delivered_to = message.get("delivered_to", [])
        if seen_by:
            label.configure(text=" ✓✓", fg="#3b82f6", font=("Segoe UI Emoji", 9, "bold"))
        elif delivered_to:
            label.configure(text=" ✓✓", fg=self.colors["muted"], font=("Segoe UI Emoji", 9, "bold"))
        else:
            label.configure(text=" ✓", fg=self.colors["muted"], font=("Segoe UI Emoji", 9))

    def _update_message_status(self, message: Dict) -> None:
        """Update only the ticks for an existing message widget."""
        handles = self.message_widgets.get(message["message_id"])
        if not handles or not handles["tick"]:
            return
        self._apply_tick_styles(handles["tick"], message)
    ...
    def _open_settings_dialog(self) -> None:
        ...
        # Status
        tk.Label(dialog, text="Status message:", bg=self.colors["panel"], fg=self.colors["text"]).pack(anchor="w", padx=20, pady=(20, 5))
        status_var = tk.StringVar(value=self.peer.config.get("status_message", "Available"))
        tk.Entry(dialog, textvariable=status_var, font=("Segoe UI", 10), bg=self.colors["composer"], fg=self.colors["text"]).pack(fill=tk.X, padx=20, ipady=4)

        # Avatar
        tk.Label(dialog, text="Avatar / initials:", bg=self.colors["panel"], fg=self.colors["text"]).pack(anchor="w", padx=20, pady=(15, 5))
        avatar_var = tk.StringVar(value=self.peer.config.get("avatar", "LL"))
        tk.Entry(dialog, textvariable=avatar_var, font=("Segoe UI", 10), bg=self.colors["composer"], fg=self.colors["text"]).pack(fill=tk.X, padx=20, ipady=4)

        # DND
        dnd_var = tk.BooleanVar(value=bool(self.peer.config.get("do_not_disturb", False)))
        tk.Checkbutton(dialog, text="Enable Do Not Disturb (Mute Sounds)", variable=dnd_var, bg=self.colors["panel"], fg=self.colors["text"], selectcolor=self.colors["panel"]).pack(anchor="w", padx=20, pady=(15, 5))

        # Theme Dropdown
        theme_var = tk.StringVar(value=self.peer.config.get("theme", "light"))
        tk.Label(dialog, text="Theme:", bg=self.colors["panel"], fg=self.colors["text"]).pack(anchor="w", padx=20, pady=(15, 5))
        theme_combo = ttk.Combobox(dialog, textvariable=theme_var, values=["light", "dark"], state="readonly", font=("Segoe UI", 10))
        theme_combo.pack(fill=tk.X, padx=20)

        def save():
            self.peer.update_profile(
                status_message=status_var.get(),
                avatar=avatar_var.get() or "LL",
                do_not_disturb=dnd_var.get(),
                theme=theme_var.get()
            )
            if theme_var.get() != self.theme_name:
                messagebox.showinfo("Theme Updated", "Restart the app to fully apply the new theme.", parent=dialog)
            dialog.destroy()

        tk.Button(dialog, text="Save Settings", command=save, bg=self.colors["send_btn"], fg="white", font=("Segoe UI", 10, "bold"), relief=tk.FLAT, padx=16, pady=8).pack(pady=25)
    ...
    def _close_active_chat(self) -> None:
        self.current_id = None
        self.current_kind = None
    ...
    def _delete_context_action(self) -> None:
        if self.current_kind == "group":
            self._delete_group_dialog()
        elif self.current_kind == "peer":
            self._delete_peer_dialog()

    def _delete_peer_dialog(self) -> None:
        if not self.current_id or self.current_kind != "peer":
            return
        if messagebox.askyesno("Remove Peer", "Are you sure you want to remove this peer and erase chat history?", parent=self.root):
    ...
    def _toggle_block_action(self) -> None:
        if not self.current_id or self.current_kind != "peer":
            return
        is_blocked = self.current_id in self.peer.config.get("blocked_peers", [])
        if is_blocked:
            self.peer.unblock_peer(self.current_id)
            self.block_btn.configure(text="🚫 Block")
            messagebox.showinfo("Unblocked", f"Peer {self.current_id} unblocked.", parent=self.root)
        else:
            if messagebox.askyesno("Block Peer", "Are you sure you want to block this peer? You will not receive any more messages from them.", parent=self.root):
                self.peer.block_peer(self.current_id)
                self.block_btn.configure(text="🛡️ Unblock")
                self._close_active_chat()
                messagebox.showinfo("Blocked", f"Peer {self.current_id} blocked.", parent=self.root)
    ...
    def _poll_events(self) -> None:
        ...
        while not self.peer.event_queue.empty():
            ...
            name = event["event"]
            data = event["data"]
            if name in {"message", "reaction", "seen", "delivered"}:
                ...
                if chat_id and chat_id == self._current_chat_id():
                    ...
                    self._render_messages([data] if name == "message" else [])
                    if name == "message" and data.get("sender_id") != self.peer.peer_id:
                        self._play_notification()
                        # Auto-mark as seen if chat is open
                        self.root.after(50, self._mark_chat_as_seen)
                    if name in {"seen", "delivered"}:
                        self._update_message_status(data)

            if name in {"peer_online", "peer_offline", "group_update"}:
                needs_sidebar_refresh = True
            
            if name == "group_kicked":
                group_name = data.get("group_name", "a group")
                messagebox.showwarning("Kicked", f"You have been removed from the group: {group_name}", parent=self.root)
                needs_sidebar_refresh = True
                if self.current_id == data.get("group_id"):
                    self._close_active_chat()
        ...
        if self._discovery_counter >= 30:
            ...
            current_ids = set(self.peer.discovery.get_peers().keys())
            if current_ids != self._last_peer_ids:
                self._last_peer_ids = current_ids
                needs_sidebar_refresh = True

        if needs_message_render:
            self._render_messages()
