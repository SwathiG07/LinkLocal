...
class GroupManager:
    ...
    def create_group(
        ...
        *,
        ...
        self.groups[group.group_id] = group
        self.peer._emit("group_update", {"group_id": group.group_id})
        return group
    ...
    async def kick_member(self, group_id: str, target_peer_id: str) -> None:
        group = self.get_group(group_id)
        if group.admin_peer_id == self.peer.peer_id:
            # Send notification to the peer BEFORE broadcasting state
            try:
                payload = self.peer._base_payload("group_meta")
                payload.update({"action": "kick_notification", "group_id": group_id, "group_name": group.group_name})
                await self.peer.send_command(target_peer_id, payload, require_encryption=False)
            except Exception:
                pass 
    ...
    async def _handle_group_meta(self, payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if action == "kick_notification":
            self.delete_group(group_id)
            self.peer._emit("group_kicked", {"group_id": group_id, "group_name": payload.get("group_name", "a group")})
            return {"ok": True}
    ...
    async def _handle_superuser_query(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if action == "kick_member":
            ...
            try:
                # Notify the member BEFORE removing them from the state
                if target_id:
                    try:
                        notif = self.peer._base_payload("group_meta")
                        notif.update({"action": "kick_notification", "group_id": group_id, "group_name": group.group_name})
                        asyncio.create_task(self.peer.send_command(target_id, notif, require_encryption=False))
                    except Exception:
                        pass
                
                group.kick(target_id, payload["sender_id"])
            except PermissionError as exc:
                return {"ok": False, "error": str(exc)}
            self.groups[group_id] = group
            await self._broadcast_state(group, action="state_update")
